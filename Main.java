...
class MazeSolver {
    ...
    public void run() {
        try {
            path = new ArrayList<>();
            String content = new String(Files.readAllBytes(Paths.get("Maze3.txt")));
            content = content.replaceAll("\r", "");
            maze = content.split("\n");
            maze = translateWalls(maze);
            Coordinate startCoordinate = new Coordinate(startRow, startCol);
            printMaze();
            startSearch(startCoordinate);
        } catch (IOException e) {
    ...
    private boolean startSearch(Coordinate currentCoordinate) {
